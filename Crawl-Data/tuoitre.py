...
def getPostData(url):
    ...
    if req.ok:
        ...
        title =  pesudo.select_one("h1#object_title") or pesudo.select_one("div.block-feature.block-feature-1 h1.title-2") or pesudo.select_one("div.title-2 clearfix + h1")
        temp["title"] = title .prettify()

        sub_tittle = pesudo.select_one("article.fck header b") or  pesudo.select_one("h2.txt-head")
        temp["sub_tittle"] = sub_tittle.prettify()

        content = pesudo.select_one("div.fck") or pesudo.select_one("div.detailfck") or pesudo.select_one("article.fck")
        temp["content"] = content.prettify()

        pub_date = pesudo.select_one("span.date")
